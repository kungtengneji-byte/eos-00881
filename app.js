...
(() => {
  ...
  // 大盤只有三級，借用同一組色階的 1／3／5 以免兩頁的顏色語意不一致
  const MARKET_RANK = { "賣壓風險升高": 1, "可續買但進入賣壓測試區": 3, "續買動能明確": 5 };
  const THRESHOLDS = [35, 50, 65, 80];
  const STATUS_LABEL = {
    confirmed: "資料完整", provisional: "暫定（部分缺值）", insufficient: "資料不足",
  };

  // 版面元件（主題切換、走勢圖、分項長條、結論、表格）與大盤頁共用，
  // 見 common.js。這裡只留 00881 自己的東西。
  const { $, el, fmt, pct, smart, rect, table } = EOSUI;
  ...
  function renderDims(row) {
    EOSUI.barList($("#dims"), Object.entries(DIMS).map(([k, name]) => ({
      key: k, name, max: DIM_MAX[k],
      earned: row[k],
      avail: row[`${k}_avail`],
    })));
  }
  ...
  function renderChart(rows) {
    EOSUI.scoreChart($("#chart-wrap"), rows, {
      ...
      tooltip: (r) =>
        ...
        '<span class="subtle">覆蓋率 ' + fmt(r.coverage, 0) +
        "　收盤 " + fmt(r.close) + "</span>",
      // 在圖上滑過等於選日期：手機上比再捲到下面那個下拉選單快得多
  ...
  async function main() {
    ...
    EOSUI.crossSummary($("#cross-card"), $("#cross-body"), "TWMARKET", {
      ...
      note: (r) => "外資續買燈號依外資現貨買波、期貨未平倉、融資與前高壓力評分，" +
                   "滿分 100，" + r.date + " 覆蓋率 " + fmt(r.coverage, 0) + "/100。",
    });
  }
